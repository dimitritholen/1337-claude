"""API keys for the 1337 plugin: the environment first, then one file.
...
The file is ~/.config/1337/credentials, NAME=value per line, mode 0600.
A file readable by group or others is refused, so a key never leaks through
a careless chmod. CLAUDE_1337_CREDENTIALS points elsewhere for tests.
Stdlib only.
"""
...
DEFAULT_PATH = os.path.join("~", ".config", "1337", "credentials")
